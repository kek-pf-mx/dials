...
def run_single(experiments1, filename):
  ...
  for exp1, exp2 in zip(experiments1, experiments2):
    ...
    b1 = exp1.beam
    b2 = exp2.beam
    assert (all(abs(d1 - d2) < EPS for d1, d2 in zip(b1.get_direction(), b2.get_direction())))
    ...
    g2 = exp2.goniometer
    if g1 is not None:
      assert (all(abs(d1 - d2) < EPS for d1, d2 in zip(g1.get_rotation_axis(), g2.get_rotation_axis())))
      assert (all(abs(d1 - d2) < EPS for d1, d2 in zip(g1.get_fixed_rotation(), g2.get_fixed_rotation())))
      assert (all(abs(d1 - d2) < EPS for d1, d2 in zip(g1.get_setting_rotation(), g2.get_setting_rotation())))
    else:
      assert (g2 is None)
    ...
    index1.append((id(exp1.beam), id(exp1.detector), id(exp1.goniometer), id(exp1.scan), id(exp1.crystal)))

    index2.append((id(exp2.beam), id(exp2.detector), id(exp2.goniometer), id(exp2.scan), id(exp2.crystal)))
...
def run():
  ...
  filename_list = ['single', 'multiple_unrelated', 'multi_crystal', 'two_colour', 'multiple_sweeps', 'stills']
