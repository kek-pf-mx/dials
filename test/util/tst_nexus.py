...
def run():
  ...
  experiments1 = ExperimentListFactory.from_json_file(join(path, "refined_experiments.json"))
  ...
  assert (all(abs(d1 - d2) < EPS for d1, d2 in zip(b1.get_polarization_normal(), b2.get_polarization_normal())))
  print 'OK'
