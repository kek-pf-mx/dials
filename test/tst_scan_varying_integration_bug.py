...
def exercise_1():
  ...
  commands = [
      "dials.import %s" % " ".join(g), "dials.slice_sweep datablock.json image_range=80,90",
      "dials.find_spots datablock_80_90.json", "dials.index datablock_80_90.json strong.pickle space_group=P41212",
  ...
  assert mtz_object.column_labels()[:14] == [
      'H', 'K', 'L', 'M_ISYM', 'BATCH', 'IPR', 'SIGIPR', 'I', 'SIGI', 'BG', 'SIGBG', 'FRACTIONCALC', 'XDET', 'YDET'
  ]
  ...
  assert expected_unit_cell.is_similar_to(uctbx.unit_cell(list(batch.cell()))), (expected_unit_cell.parameters(),
                                                                                 list(batch.cell()))
