...
def any_compatible_unit_cell(space_group, volume=None, asu_volume=None):
  ...
  while True:
    ...
    if ((alpha + beta + gamma) > 0 and (alpha + beta + gamma) < 360 and (alpha + beta - gamma) > 0
        and (alpha + beta - gamma) < 360 and (alpha - beta + gamma) > 0 and (alpha - beta + gamma) < 360
        and (-alpha + beta + gamma) > 0 and (-alpha + beta + gamma) < 360):
      break
    else:
...
def generate_spots(crystal_model, detector, beam, goniometer=None, scan=None, sel_fraction=1.0):
  import math

  experiment = Experiment(beam=beam, detector=detector, goniometer=goniometer, scan=scan, crystal=crystal_model)
  ...
  imageset = ImageSweep(
      NullReader, indices=range(len(scan.get_epochs())), beam=beam, goniometer=goniometer, detector=detector, scan=scan)
  experiment.imageset = imageset

  predicted = flex.reflection_table.from_predictions(experiment)

  sel = flex.random_selection(len(predicted), int(math.floor(sel_fraction * len(predicted))))
  ...
  predicted['xyzobs.px.variance'] = flex.vec3_double(len(predicted), (0.5, 0.5, 0.5))
  return predicted
...
def generate_crystal(unit_cell, space_group):
  ...
  return Crystal(direct_matrix[0:3], direct_matrix[3:6], direct_matrix[6:9], space_group=space_group)
...
def run_indexing(datablock, strong_spots, crystal_model, rmsds):
  ...
  expected_rmsds = [1.1 * rmsds[0] * pixel_size[0], 1.1 * rmsds[1] * pixel_size[1], 1.1 * rmsds[2] * phi_width]
...
def add_random_noise_xyz(datablock, strong_spots, rmsds):
  ...
  errors = flex.vec3_double(*list(
      scitbx.random.variate(scitbx.random.normal_distribution(0, rmsds[i]))(len(strong_spots)) for i in range(3)))
  strong_spots['xyzobs.px.value'] += errors

def run():

  #space_group = sgtbx.space_group_info(number=random.randint(1,230)).group()
  space_group = sgtbx.space_group_info(number=random.choice((1, 3, 16, 75, 143, 195))).group()

  unit_cell = any_compatible_unit_cell(space_group, volume=random.uniform(1e3, 1e6))
