...
def unit_cells_are_similar(uc1, uc2, relative_length_tolerance=0.01, absolute_angle_tolerance=1):
...
class run_one_indexing(object):
  def __init__(self,
    ...
    experiments_list = dxtbx_load.experiment_list(os.path.join(tmp_dir, "experiments.json"), check_format=False)
    assert len(experiments_list.crystals()) == n_expected_lattices, (len(experiments_list.crystals()),
                                                                     n_expected_lattices)
    ...
    for i in range(len(experiments_list)):
      ...
      assert unit_cells_are_similar(
          ...
          expected_unit_cell,
          relative_length_tolerance=relative_length_tolerance,
          absolute_angle_tolerance=absolute_angle_tolerance), (self.crystal_model.get_unit_cell().parameters(),
                                                               expected_unit_cell.parameters())
      sg = self.crystal_model.get_space_group()
      assert sg.type().hall_symbol() == expected_hall_symbol, (sg.type().hall_symbol(), expected_hall_symbol)
      reflections = self.indexed_reflections.select(self.indexed_reflections['id'] == i)
  ...
  def get_rmsds_obs_pred(self, observations, experiment):
    reflections = observations.select(observations.get_flags(observations.flags.used_in_refinement))
...
def test_exercise_1(dials_regression):
  ...
  extra_args = ["bin_size_fraction=0.25", "scan_range=1,20", "scan_range=250,270", "scan_range=520,540"]
  expected_unit_cell = uctbx.unit_cell((58, 58, 150, 90, 90, 90))
  expected_rmsds = (0.05, 0.04, 0.0005)
  expected_hall_symbol = ' P 1'

  result = run_one_indexing(pickle_path, sweep_path, extra_args, expected_unit_cell, expected_rmsds,
                            expected_hall_symbol)

def test_exercise_2(dials_regression):
  missing = check_external_dependencies(['scipy', 'sklearn', 'networkx'])
  if len(missing):
    print("Skipping test_exercise_2: missing dependencies " + ", ".join(missing))
    return
  # thaumatin
  data_dir = os.path.join(dials_regression, "indexing_test_data", "i04_weak_data")
  pickle_path = os.path.join(data_dir, "full.pickle")
  sweep_path = os.path.join(data_dir, "datablock_orig.json")
  extra_args = [
      "cluster_analysis_search=True", "n_macro_cycles=3", "bin_size_fraction=0.25", "reciprocal_space_grid.d_min=4"
  ]
  expected_unit_cell = uctbx.unit_cell((58, 58, 150, 90, 90, 90))
...
def test_exercise_3(dials_regression):
  ...
  missing = check_external_dependencies(['scipy', 'sklearn', 'networkx'])
  if len(missing):
    print("Skipping test_exercise_3: missing dependencies " + ", ".join(missing))
    return
  # thaumatin
  data_dir = os.path.join(dials_regression, "indexing_test_data", "i04_weak_data")
  pickle_path = os.path.join(data_dir, "full.pickle")
...
def test_exercise_4(dials_regression):
  missing = check_external_dependencies(['scipy', 'sklearn', 'networkx'])
  if len(missing):
    print("Skipping test_exercise_4: missing dependencies " + ", ".join(missing))
    return
  # trypsin
...
def test_exercise_5(dials_regression):
  missing = check_external_dependencies(['scipy', 'sklearn', 'networkx'])
  if len(missing):
    print("Skipping test_exercise_5: missing dependencies " + ", ".join(missing))
    return
  ...
  extra_args = [
      "cluster_analysis_search=True", "reflections_per_degree=10", "n_macro_cycles=2", "reciprocal_space_grid.d_min=4",
      "max_cell=70", "scan_range=0,50", "scan_range=450,500", "scan_range=850,900"
  ]
  expected_unit_cell = uctbx.unit_cell((54.3, 58.3, 66.5, 90, 90, 90))
...
def test_exercise_6(dials_regression):
  missing = check_external_dependencies(['scipy', 'sklearn', 'networkx'])
  if len(missing):
    print("Skipping test_exercise_6: missing dependencies " + ", ".join(missing))
    return
...
def test_exercise_7(dials_regression):
  missing = check_external_dependencies(['scipy', 'sklearn', 'networkx'])
  if len(missing):
    print("Skipping test_exercise_7: missing dependencies " + ", ".join(missing))
    return
...
def test_exercise_9(dials_regression):
  ...
  sweep_path = os.path.join(data_dir, "datablock_orig.json")
  extra_args = [
      "n_macro_cycles=2", "indexing.method=fft1d", "bin_size_fraction=0.25", "scan_range=1,20", "scan_range=250,270",
      "scan_range=520,540"
  ]
  expected_unit_cell = uctbx.unit_cell((58, 58, 150, 90, 90, 90))
...
def test_exercise_12(dials_regression):
  missing = check_external_dependencies(['scipy', 'sklearn', 'networkx'])
  if len(missing):
    print("Skipping test_exercise_12: missing dependencies " + ", ".join(missing))
    return
  ...
  sweep_path = os.path.join(data_dir, "datablock_orig.json")
  extra_args = [
      "indexing.method=fft3d", "known_symmetry.space_group=P4", "known_symmetry.unit_cell=57.8,57.8,150,90,90,90",
      "peak_search=clean", "cluster_analysis_search=True", "min_samples=15", "n_macro_cycles=4",
      "reciprocal_space_grid.d_min=4"
  ]
...
def test_exercise_14(dials_regression):
  ...
  for i, image_path in enumerate(("insulin_1_001.img", "insulin_1_045.img")):
    shutil.copyfile(os.path.join(data_dir, image_path), "image_00%i.img" % (i + 1))

  args = [
      "dials.import", ' '.join(glob.glob(os.path.join(tmp_dir, "image_00*.img"))), "output.datablock=datablock.json",
  ...
  for method in ("fft3d", "fft1d", "real_space_grid_search"):
    extra_args = []
    extra_args.append("known_symmetry.unit_cell=\"%s %s %s %s %s %s\"" % expected_unit_cell.parameters())
    ...
    result = run_one_indexing(pickle_path, datablock_json, extra_args, expected_unit_cell, expected_rmsds,
                              expected_hall_symbol)
...
def test_exercise_15(dials_regression):
  ...
  extra_args = ["max_refine=10", "reflections_per_degree=50", "known_symmetry.space_group=R3", "n_macro_cycles=3"]
...
def test_exercise_16(dials_regression):
  ...
  pickle_paths = [
      glob.glob(os.path.join(data_dir, "SWEEP%i" % (i + 1), "index", "*_strong.pickle"))[0] for i in range(4)
  ]
  sweep_paths = [
      glob.glob(os.path.join(data_dir, "SWEEP%i" % (i + 1), "index", "*_datablock_import.json"))[0] for i in range(4)
  ]
  ...
  result = run_one_indexing(" ".join(pickle_paths), " ".join(sweep_paths), extra_args, expected_unit_cell,
...
def test_exercise_17(dials_regression):
  ...
  pickle_paths = [glob.glob(os.path.join(data_dir, "*SWEEP%i*_strong.pickle" % (i + 1)))[0] for i in range(3)]
  sweep_paths = [glob.glob(os.path.join(data_dir, "*SWEEP%i*_datablock.json" % (i + 1)))[0] for i in range(3)]
  extra_args = ["filter_ice=False"]
  expected_unit_cell = uctbx.unit_cell((9.440, 15.313, 17.126, 90.073, 90.106, 79.248))
...
def test_exercise_18(dials_regression):
  ...
  result = run_one_indexing(pickle_path, datablock, extra_args, expected_unit_cell, expected_rmsds,
                            expected_hall_symbol)
...
def run(args):
  ...
  exercises = (test_exercise_1, test_exercise_2, test_exercise_3, test_exercise_4, test_exercise_5, test_exercise_6,
               test_exercise_7, test_exercise_8, test_exercise_9, test_exercise_10, test_exercise_11, test_exercise_12,
               test_exercise_13, test_exercise_14, test_exercise_15, test_exercise_16, test_exercise_17,
               test_exercise_18)
  if len(args):
  ...
  easy_mp.parallel_map(func=run_parallel, iterable=[(e, ) for e in exercises], processes=nproc)
