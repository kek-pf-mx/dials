...
class Test(object):
  ...
  def tst_vec2_double(self, num):
    ...
    for i in range(num):
      ...
      yy0 = randint(y0, y1)
      yy1 = randint(y0, y1)
      rg = (min(xx0, xx1), max(xx0, xx1) + 1, min(yy0, yy1), max(yy0, yy1) + 1)
  ...
  def tst_vec3_double(self, num):
    ...
    x0, x1, y0, y1, z0, z1 = (int(min(x)), int(max(x)), int(min(y)), int(max(y)), int(min(z)), int(max(z)))
    for i in range(num):
      xx0 = randint(x0, x1)
      xx1 = randint(x0, x1)
      yy0 = randint(y0, y1)
      ...
      zz0 = randint(y0, y1)
      zz1 = randint(y0, y1)
      rg = (min(xx0, xx1), max(xx0, xx1) + 1, min(yy0, yy1), max(yy0, yy1) + 1, min(zz0, zz1), max(zz0, zz1) + 1)
      idx = index.query_range(rg)
      for j in idx:
  ...
  def inside2d(self, rg, a):
    inside = (rg[0] <= a[0] and rg[1] >= a[0] and rg[2] <= a[1] and rg[3] >= a[1])
    return inside

  def inside3d(self, rg, a):
    inside = (rg[0] <= a[0] and rg[1] >= a[0] and rg[2] <= a[1] and rg[3] >= a[1] and rg[4] <= a[2] and rg[5] >= a[2])
    return inside
