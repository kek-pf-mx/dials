...
def assert_basic_mask_is_correct(mask, ninvalid, nforeground):
  ...
  foreground = flex.size_t(i for i in range(len(mask)) if mask[i] & MaskCode.Foreground)
  background = flex.size_t(i for i in range(len(mask)) if mask[i] & MaskCode.Background)
  background_used = flex.size_t(i for i in range(len(mask)) if mask[i] & MaskCode.BackgroundUsed)
  background_valid = flex.size_t(
      i for i in range(len(mask)) if mask[i] & MaskCode.Valid and mask[i] & MaskCode.Background)
...
class TestTruncated(object):
  ...
  def run(self):
    for i in range(10):
      data, mask = generate_shoebox(self.size, self.mean, self.nforeground, self.ninvalid)
