...
def exercise_merge_spot_lists():
  ...
  args = ["dials.spotfinder", template, "-o spotfinder.pickle", "min_spot_size=0", "max_separation=100"]
  result = easy_run.fully_buffered(command=" ".join(args)).raise_if_errors()
  assert os.path.exists("spotfinder.pickle")

  template = os.path.join(data_dir, "centroid_00*.cbf")
  args = [
      "dials.spotfinder", template, "-o spotfinder1.pickle", "min_spot_size=0", "max_separation=100",
      'scan_range="0, 4"'
  ]
  result = easy_run.fully_buffered(command=" ".join(args)).raise_if_errors()
  ...
  template = os.path.join(data_dir, "centroid_00*.cbf")
  args = [
      "dials.spotfinder", template, "-o spotfinder2.pickle", "min_spot_size=0", "max_separation=100",
      'scan_range="4, 9"'
  ]
  result = easy_run.fully_buffered(command=" ".join(args)).raise_if_errors()
  assert os.path.exists("spotfinder2.pickle")

  args = ["dials.merge_spot_lists", 'spotfinder1.pickle', 'spotfinder2.pickle', "-o spotfinder3.pickle"]
  result = easy_run.fully_buffered(command=" ".join(args)).raise_if_errors()
