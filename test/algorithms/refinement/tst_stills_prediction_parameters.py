...
class Test(object):
  ...
  def create_models(self):
    ...
    self.stills_experiments.append(
        Experiment(beam=self.beam, detector=self.detector, crystal=self.crystal, imageset=None))
  ...
  def generate_reflections(self):
    ...
    sf = ScanFactory()
    self.scan = sf.make_scan(image_range=(1, 1), exposure_times=0.1, oscillation=(0, 3.0), epochs=range(1), deg=True)
  ...
  def get_fd_gradients(self, pred_param, ref_predictor):
    ...
    for i in range(len(deltas)):
      ...
      fd_grads.append({'name': p_names[i], 'dX_dp': x_grads, 'dY_dp': y_grads, 'dDeltaPsi_dp': delpsi_grads})
...
"absolute error: {1:9.6f}").format(n_outliers, tst_val)
        ...
        try:
          ...
          e.args += ("extreme normalised error value: {0}".format(flex.max(flex.abs(norm_error))), )
          raise e
  ...
  def run_spherical_relp_stills_pred_param(self, verbose=True):
    ...
    # the analytical gradients so quantities like s1 are correct
    from dials.algorithms.refinement.prediction import ExperimentsPredictor
    ref_predictor = ExperimentsPredictor(self.stills_experiments, spherical_relp=True)
    ref_predictor(self.reflections)
