...
def random_panel(lim=(0, 50)):
  ...
  o = matrix.col((random.uniform(-200, 200), random.uniform(-200, 200), random.uniform(-200, 200)))
  ...
  return Panel("PAD", "Panel", u1, u2, o, (lim[1] / 200, lim[1] / 200), (200, 200), (0, 2e20), 0.0, "")
...
if __name__ == '__main__':
  ...
  detector = DetectorFactory.make_detector("PAD", d1, d2, matrix.col((0, 0, -110)), (pix_size_f, pix_size_s),
                                           (npx_fast, npx_slow), (0, 2e20))

  dp = DetectorParameterisationSinglePanel(detector)
  beam = BeamFactory().make_beam(sample_to_source=-1 * (matrix.col((0, 0, -110)) + 10 * d1 + 10 * d2), wavelength=1.0)
  ...
  v2 = matrix.col((sin(pi / 18)**2, -cos(pi / 18),
  ...
  for i in range(attempts):
    ...
    p_vals = dp.get_param_vals()
    p_vals = random_param_shift(p_vals, [10, 10, 10, 1000. * pi / 18, 1000. * pi / 18, 1000. * pi / 18])
    dp.set_param_vals(p_vals)
    ...
    for j in range(6):
      try:
        assert (approx_equal((fd_ds_dp[j] - an_ds_dp[j]), matrix.sqr((0., 0., 0., 0., 0., 0., 0., 0., 0.)), eps=1.e-6))
  ...
  beam = BeamFactory().make_beam(sample_to_source=-1. * beam_centre, wavelength=1.0)
  ...
  for i in range(attempts):
    ...
    for j in range(9):
      ...
      fd_ds_dp = get_fd_gradients(dp, [1.e-7] * dp.num_free(), multi_state_elt=j)
  ...
  if failures == 0: print "OK"
    ...
    raise RuntimeError("Failure comparing analytical with finite difference" "derivatives. Check stdout for details.")
