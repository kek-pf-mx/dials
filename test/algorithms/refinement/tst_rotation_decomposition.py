...
def test_rotation_matrices(phi1, phi2, phi3):
  ...
  sol1 = solve_r3_rotation_for_angles_given_axes(R, (1, 0, 0), (0, 1, 0), (0, 0, 1), smaller_phi2_solution=False)
  sol2 = solve_r3_rotation_for_angles_given_axes(R, (1, 0, 0), (0, 1, 0), (0, 0, 1), smaller_phi2_solution=True)

  # recompose these into rotation matrices
  tstR1 = matrix.col((1, 0, 0)).axis_and_angle_as_r3_rotation_matrix(sol1[0], deg=False)
  tstR1 *= matrix.col((0, 1, 0)).axis_and_angle_as_r3_rotation_matrix(sol1[1], deg=False)
  tstR1 *= matrix.col((0, 0, 1)).axis_and_angle_as_r3_rotation_matrix(sol1[2], deg=False)

  tstR2 = matrix.col((1, 0, 0)).axis_and_angle_as_r3_rotation_matrix(sol2[0], deg=False)
  tstR2 *= matrix.col((0, 1, 0)).axis_and_angle_as_r3_rotation_matrix(sol2[1], deg=False)
  tstR2 *= matrix.col((0, 0, 1)).axis_and_angle_as_r3_rotation_matrix(sol2[2], deg=False)
...
def test_vs_euler_angles_xyz_angles(phi1, phi2, phi3):
  ...
  assert any([approx_equal(sol1, tst, out=None), approx_equal(sol2, tst, out=None)])
  return

def random_vector():
  v = matrix.col((1, 0, 0)).rotate_around_origin(matrix.col((0, 1, 0)), random.uniform(0, math.pi))
  return v.rotate_around_origin(matrix.col((1, 0, 0)), random.uniform(0, 2. * math.pi)).normalize()
