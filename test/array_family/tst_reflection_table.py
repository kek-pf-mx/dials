...
class Test(object):
  ...
  def tst_init(self):
    ...
    table = flex.reflection_table([('col1', flex.int(10)), ('col2', flex.double(10)), ('col3', flex.std_string(10))])
    assert (table.is_consistent())
    assert (table.nrows() == 10)
    ...
    try:
      table = flex.reflection_table([('col1', flex.int(10)), ('col2', flex.double(20)), ('col3', flex.std_string(10))])
      assert (false)
    except Exception:
      pass
    print 'OK'
      ...
      for z in range(z1 - z0):
        for y in range(y1 - y0):
          for x in range(x1 - x0):
            v1 = data[z, y, x]
            m1 = mask[z, y, x]
            if (x0 + x >= 0 and y0 + y >= 0 and x0 + x < width and y0 + y < height):
  ...
  def tst_split_indices_by_experiment_id(self):
    ...
    for index, exp, num in zip(index_list, [0, 1, 2, 3, 4, 5], [100, 100, 100, 100, 0, 100]):
  ...
  def tst_split_partials(self):
    ...
    for i in range(100):
      ...
      v2 = randint(0, 100)
      v3 = uniform(0, 100)
      r.append({'value1': v1, 'value2': v2, 'value3': v3, 'bbox': (x0, x1, y0, y1, z0, z1)})
  ...
  def tst_split_partials_with_shoebox(self):
    ...
    for i in range(100):
      ...
      r.append(
          {'value1': v1, 'value2': v2, 'value3': v3, 'bbox': (x0, x1, y0, y1, z0, z1), 'panel': 0, 'shoebox': sbox})
      for z in range(z0, z1):
        ...
        expected.append({
            'value1': v1, 'value2': v2, 'value3': v3, 'bbox': (x0, x1, y0, y1, z, z + 1), 'partial_id': i, 'panel': 0,
            'shoebox': sbox
        })
    ...
    for r1, r2 in zip(r, expected):
      ...
      assert (r1['shoebox'].data.as_double().as_1d().all_approx_equal(r2['shoebox'].data.as_double().as_1d()))
  ...
  def tst_find_overlapping(self):
    ...
    def is_overlap(b0, b1, border):
      ...
      if not (b1[0] > b0[1] or b1[1] < b0[0] or b1[2] > b0[3] or b1[3] < b0[2] or b1[4] > b0[5] or b1[5] < b0[4]):
        return True
      return False
