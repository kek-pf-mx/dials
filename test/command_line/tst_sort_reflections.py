...
class Test(object):
  ...
  def run(self):
    ...
    easy_run.fully_buffered([
        'dev.dials.sort_reflections', input_filename, 'output=sorted2.pickle', 'key=intensity.sum.value', 'reverse=True'
    ]).raise_if_errors()
    ...
    easy_run.fully_buffered(['dev.dials.sort_reflections', input_filename, 'output=sorted3.pickle']).raise_if_errors()
