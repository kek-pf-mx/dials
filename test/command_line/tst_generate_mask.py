...
class Test(object):
  ...
  def run(self):
    ...
    # Call dials.generate_mask
    easy_run.fully_buffered([
        'dials.generate_mask', input_filename, 'output.mask=mask2.pickle', 'output.datablock=masked_datablock.json',
    ...
    import os
    assert imageset.external_lookup.mask.filename == os.path.join(os.path.abspath(os.getcwd()), 'mask2.pickle')
    ...
    easy_run.fully_buffered(
        ['dials.generate_mask', input_filename, 'output.mask=mask3.pickle',
         'untrusted.circle=100,100,10']).raise_if_errors()
    ...
    easy_run.fully_buffered([
        'dials.generate_mask', input_filename, 'output.mask=mask3.pickle',
        'untrusted.polygon=100,100,100,200,200,200,200,100', 'untrusted.pixel=0,0', 'untrusted.pixel=1,1'
    ]).raise_if_errors()
    assert (exists("mask3.pickle"))
