...
def run():
  ...
  commands = ["dials.reindex", pickle_path, experiments_path, "change_of_basis_op=2a,b,c", "space_group=P1"]
  ...
  new_experiments = load.experiment_list('reindexed_experiments.json', check_format=False)
  ...
  commands = ["dials.reindex", "P4.json", "change_of_basis_op=%s" % str(cb_op), "output.experiments=P4_reindexed.json"]
  command = " ".join(commands)
  print command
  result = easy_run.fully_buffered(command=command).raise_if_errors()
  new_experiments1 = load.experiment_list('P4_reindexed.json', check_format=False)
  assert approx_equal(new_experiments1[0].crystal.get_A(), old_experiments[0].crystal.change_basis(cb_op).get_A())
  #
  cb_op = sgtbx.change_of_basis_op('-x,-y,z')
  commands = [
      "dials.reindex", "P4.json", "change_of_basis_op=auto", "reference=P4_reindexed.json",
  ...
  assert approx_equal(new_experiments1[0].crystal.get_A(), new_experiments2[0].crystal.get_A())
