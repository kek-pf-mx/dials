...
def exercise_spotfinder():
  ...
  args = [
      "dials.find_spots", "filter.d_min=2", "filter.d_max=15", ' '.join(template),
  ...
  args = [
      "dials.find_spots", "write_hot_mask=True", ' '.join(template), "output.reflections=spotfinder.pickle",
  ...
  # now write a hot mask
  args = [
      "dials.find_spots", "write_hot_mask=True", "hot_mask_prefix=my_hot_mask", ' '.join(template),
      "output.reflections=spotfinder.pickle", "output.shoeboxes=False"
  ]
  result = easy_run.fully_buffered(command=" ".join(args)).raise_if_errors()
  assert os.path.exists("spotfinder.pickle")
  ...
  args = [
      "dials.find_spots", "min_spot_size=3", "max_separation=3", ' '.join(template),
  ...
  template = glob(os.path.join(data_dir, "centroid*.cbf"))
  args = [
      "dials.find_spots", ' '.join(template), "output.reflections=spotfinder.pickle", "output.shoeboxes=True",
  ...
  data_dir = libtbx.env.find_in_repositories(relative_path="dials_regression/spotfinding_test_data", test=os.path.isdir)
