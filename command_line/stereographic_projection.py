...
def run(args):
  ...
  if params.frame == 'laboratory':
    reference_poles = reference_poles_perpendicular_to_beam(experiments[0].beam, experiments[0].goniometer)
    if params.use_starting_angle:
      rotation_axis = matrix.col(experiments[0].goniometer.get_rotation_axis())
      R = rotation_axis.axis_and_angle_as_r3_rotation_matrix(experiments[0].scan.get_oscillation()[0], deg=True)
    elif params.phi_angle != 0:
      rotation_axis = matrix.col(experiments[0].goniometer.get_rotation_axis())
      R = rotation_axis.axis_and_angle_as_r3_rotation_matrix(params.phi_angle, deg=True)
  else:
    if params.plane_normal is not None:
      plane_normal = params.plane_normal
    else:
      plane_normal = (0, 0, 1)
    reference_poles = reference_poles_crystal(ref_crystal, plane_normal=plane_normal)
  ...
  if experiments:
    ...
    for expt in experiments[1:]:
      cryst = expt.crystal
      if params.frame == 'crystal':
        R_ij, axis, angle, cb_op = difference_rotation_matrix_axis_angle(ref_crystal, cryst)
        U = R_ij
      elif params.use_starting_angle:
        if params.use_starting_angle:
          rotation_axis = matrix.col(expt.goniometer.get_rotation_axis())
          R = rotation_axis.axis_and_angle_as_r3_rotation_matrix(expt.scan.get_oscillation()[0], deg=True)
      else:
...
def plot_projections(projections,
  ...
  if gridsize is not None:
    ...
    for i, projections in enumerate(projections_all):
      ...
      pyplot.scatter(x.as_numpy_array(), y.as_numpy_array(), c=colours[i], s=marker_size, edgecolors='none')
      if label_indices:
...
def projections_as_dict(projections):
  ...
  data.append({
      ...
      'y': [0],
      'mode': 'markers',
      'marker': {'color': 'black', 'size': 25, 'symbol': 'cross-thin', 'line': {'width': 1}},
      'showlegend': False,
  })
  ...
  d = {
      ...
      'layout': {
          'title':
          'Stereographic projections', 'hovermode':
          False, 'xaxis': {
              'range': [-1.0, 1.0],
              'showgrid': False,
              'zeroline': False,
              ...
              'type': 'circle', 'xref': 'x', 'yref': 'y', 'x0': -1, 'y0': -1, 'x1': 1, 'y1': 1, 'line': {
                  'color': 'black',
              }
          }]
      },
