...
def run(args):
  ...
  for i_expt, expt in enumerate(experiments):
    ...
    rotation_matrix = rotation_axis.axis_and_angle_as_r3_rotation_matrix(scan.get_oscillation()[0], deg=True)
...
class Strategy(object):
  def __init__(self, experiment, other=None, d_min=None, unit_cell_scale=1, degrees_per_bin=5, min_frac_new=0.001):
    ...
    scan.set_image_range((image_range[0], image_range[0] + int(max_rotation / oscillation[1])))
    self.predict()
    x, y, z = self.predicted['xyzcal.px'].parts()
    self.predicted['dose'] = flex.size_t(list(flex.ceil(z * oscillation[1] / self.degrees_per_bin).iround()))
  ...
  def predict(self):
    ...
    self.predicted = flex.reflection_table.from_predictions(self.experiment, force_static=True, dmin=self.d_min)
  ...
  def determine_cutoffs(self, min_frac_new=None):
    if min_frac_new is None:
      self.min_frac_new = min_frac_new
    self.cutoff_non_anom = self.degrees_per_bin * self.determine_fraction_new_cutoff(self.frac_new_ref, min_frac_new)
    self.cutoff_anom = self.degrees_per_bin * self.determine_fraction_new_cutoff(self.frac_new_pairs, min_frac_new)
  ...
  def show(self):
    ...
    print "  (completeness: %.2f %%)" % (100 * self.iboth_completeness[int(self.cutoff_anom / self.degrees_per_bin)])

  def plot(self, prefix=''):
    plot_statistics(
        self.stats,
        prefix=prefix,
        degrees_per_bin=self.degrees_per_bin,
...
class ComputeStats(object):
  def __init__(self, strategies, n_bins=8, degrees_per_bin=5):
    ...
    cs = crystal.symmetry(unit_cell=strategies[0].experiment.crystal.get_unit_cell(), space_group=sg)
    ...
    ma = miller.array(ms, data=flex.double(ms.size(), 1), sigmas=flex.double(ms.size(), 1))
    if 1:
    ...
    binner = ma.setup_binner_d_star_sq_step(d_star_sq_step=(flex.max(d_star_sq) - flex.min(d_star_sq) + 1e-8) / n_bins)
    ...
    chef_stats = ChefStatistics(ma.indices(), ma.data(), ma.sigmas(),
                                ma.d_star_sq().data(), dose, self.n_complete, binner, ma.space_group(),
...
def plot_statistics(statistics, prefix='', degrees_per_bin=5, cutoff_anom=None, cutoff_non_anom=None):
