...
class render_3d(object):
  ...
  def load_models(self, imagesets, reflections, crystals):
    ...
    if self.imagesets[0].get_goniometer() is not None:
      self.viewer.set_rotation_axis(self.imagesets[0].get_goniometer().get_rotation_axis())
    ...
    for i, imageset in enumerate(self.imagesets):
      ...
      if 'xyzobs.px.value' in self.reflections_input:
        refl = indexer.indexer_base.map_spots_pixel_to_mm_rad(
            self.reflections_input.select(sel), imageset.get_detector(), imageset.get_scan())
        ...
        indexer.indexer_base.map_centroids_to_reciprocal_space(
            refl, imageset.get_detector(), imageset.get_beam(), goniometer, calculated=True)
  ...
  def set_points(self):
    ...
    if 'miller_index' in reflections:
      if 'flags' not in reflections:
        ...
        reflections.set_flags(flex.bool(len(reflections), True), reflections.flags.strong)
        reflections.set_flags(flex.bool(len(reflections), False), reflections.flags.integrated)
        reflections.set_flags(flex.bool(len(reflections), False), reflections.flags.centroid_outlier)
    ...
    if len(points):
      ...
      palette = flex.vec3_double(((255, 255, 255), (230, 159, 0), (86, 180, 233), (0, 158, 115), (240, 228, 66),
...
class ReciprocalLatticeViewer(wx.Frame, render_3d):
  ...
  def create_viewer_panel(self):
    ...
    self.viewer = RLVWindow(settings=self.settings, parent=self, size=(800, 600), background_rgb=background_rgb)
  ...
  def update_statusbar(self):
    ...
    txt = 'Model view matrix: ' + '[' + ', '.join('%.4f' % m for m in model_view_matrix) + ']'
...
class settings_window(wxtbx.utils.SettingsPanel):
  ...
  def add_controls(self):
    ...
    self.partiality_min_ctrl = floatspin.FloatSpin(parent=self, increment=0.01, digits=3, min_val=0, max_val=1)
    ...
    self.partiality_max_ctrl = floatspin.FloatSpin(parent=self, increment=0.01, digits=3, min_val=0, max_val=1)
    ...
    ctrls = self.create_controls(setting="show_rotation_axis", label="Show rotation axis")
    self.panel_sizer.Add(ctrls[0], 0, wx.ALL, 5)
    ctrls = self.create_controls(setting="show_beam_vector", label="Show beam vector")
    self.panel_sizer.Add(ctrls[0], 0, wx.ALL, 5)
    ctrls = self.create_controls(setting="show_reciprocal_cell", label="Show reciprocal cell")
    self.panel_sizer.Add(ctrls[0], 0, wx.ALL, 5)
    self.reverse_phi_ctrl = self.create_controls(setting="reverse_phi", label="Invert rotation axis")[0]
    ...
    self.marker_size_ctrl = floatspin.FloatSpin(parent=self, increment=1, digits=0, min_val=1)
    ...
    self.btn.SetSelection(["all", "indexed", "unindexed", "integrated"].index(self.settings.display))
    ...
    self.outlier_btn.SetSelection([None, "inliers", "outliers"].index(self.settings.outlier_display))
  ...
  def add_value_widgets(self, sizer):
    sizer.Add(wx.StaticText(self.panel, -1, "Value:"), 0, wx.ALL | wx.ALIGN_CENTER_VERTICAL, 5)
    self.value_info = wx.TextCtrl(self.panel, -1, size=(80, -1), style=wx.TE_READONLY)
  ...
  def add_experiments_buttons(self):
    ...
    for i in range(-1, n + 1):
      ...
      if (self.settings.experiment_ids is not None and i in self.settings.experiment_ids):
  ...
  def add_imagesets_buttons(self):
    ...
    for i in range(n + 1):
      ...
      if (self.settings.imageset_ids is not None and i in self.settings.imageset_ids):
  ...
  def OnChangeSettings(self, event):
    ...
    self.settings.beam_centre = (self.beam_fast_ctrl.GetValue(), self.beam_slow_ctrl.GetValue())
...
class RLVWindow(wx_viewer.show_points_and_lines_mixin):
  ...
  def set_reciprocal_lattice_vectors(self, vectors_per_crystal):
    ...
    for vec_set in vectors_per_crystal:
      self.recip_latt_vectors.append([100. * matrix.col(vec) for vec in vec_set])
  ...
  def rotate_view(self, x1, y1, x2, y2, shift_down=False, scale=0.1):
    super(RLVWindow, self).rotate_view(x1, y1, x2, y2, shift_down=shift_down, scale=scale)
