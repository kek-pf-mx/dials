...
def show_beam(detector, beam):
  ...
  if panel_id >= 0 and x is not None and y is not None:
    ...
    if len(detector) > 1:
      beam_centre_mm_str = "Beam centre (mm): panel %i, (%.2f,%.2f)" % (panel_id, x, y)
      beam_centre_px_str = "Beam centre (px): panel %i, (%.2f,%.2f)" % (panel_id, x_px, y_px)
    else:
  ...
  if beam.num_scan_points > 0:
    ...
    s += "Beam centre range (mm): ([%.2f,%.2f],[%.2f,%.2f])\n" % (min(x_mm), max(x_mm), min(y_mm), max(y_mm))
    s += "Beam centre range (px): ([%.2f,%.2f],[%.2f,%.2f])\n" % (min(x_px), max(x_px), min(y_px), max(y_px))
...
def run(args):
  ...
  usage = "%s [options] datablock.json | experiments.json | image_*.cbf" % (libtbx.env.dispatcher_name)
  ...
  if len(experiments):
    print show_experiments(
        experiments, show_panel_distance=params.show_panel_distance, show_scan_varying=params.show_scan_varying)

  if len(datablocks):
    print show_datablocks(datablocks, show_panel_distance=params.show_panel_distance)
...
def show_experiments(experiments, show_panel_distance=False, show_scan_varying=False):
  ...
  for i_expt, expt in enumerate(experiments):
    ...
    text.append('Max resolution (at corners): %f' % (expt.detector.get_max_resolution(expt.beam.get_s0())))
    text.append('Max resolution (inscribed):  %f' % (expt.detector.get_max_inscribed_resolution(expt.beam.get_s0())))
...
def show_datablocks(datablocks, show_panel_distance=False):
  ...
  for datablock in datablocks:
    ...
    for imageset in imagesets:
      try:
        text.append(imageset.get_template())
      except Exception:
        pass
      detector = imageset.get_detector()
      if detector is not None:
        text.append(str(detector))
        text.append('Max resolution (at corners): %f' % (detector.get_max_resolution(imageset.get_beam().get_s0())))
...
def _create_flag_count_table(table):
  ...
  for flag in flag_order:
    ...
    flag_rows.append([
        indent + flag.name, "{:{:d}d}".format(flag_count[flag], max_count_len), "{:5.01f}".format(
            100 * flag_count[flag] / len(table))
    ])

  # Build the array of output strings
  text = []
  text.append("Reflection flags:")
  text.append(table_utils.format(flag_rows, has_header=True, prefix="| ", postfix=" |"))
  return "\n".join(text)
...
def show_reflections(reflections,
    ...
    for k, col in rlist.cols():
      if k in formats and not "%" in formats[k]:
        ...
        if type(col) in (flex.int, flex.size_t):
          col = col.as_double()
        rows.append([k, formats[k] % flex.min(col), formats[k] % flex.max(col), formats[k] % flex.mean(col)])
      elif type(col) in (flex.vec3_double, flex.miller_index):
        if isinstance(col, flex.miller_index):
          col = col.as_vec3_double()
        rows.append([k, formats[k] % col.min(), formats[k] % col.max(), formats[k] % col.mean()])
      elif isinstance(col, flex.shoebox):
        rows.append([k, "", "", ""])
        si = col.summed_intensity().observed_value()
        rows.append(["  summed I", formats[k] % flex.min(si), formats[k] % flex.max(si), formats[k] % flex.mean(si)])
        x1, x2, y1, y2, z1, z2 = col.bounding_boxes().parts()
        bbox_sizes = ((z2 - z1) * (y2 - y1) * (x2 - x1)).as_double()
        rows.append([
            "  N pix", formats[k] % flex.min(bbox_sizes), formats[k] % flex.max(bbox_sizes),
            formats[k] % flex.mean(bbox_sizes)
        ])
        fore_valid = col.count_mask_values(foreground_valid).as_double()
        rows.append([
            "  N valid foreground pix", formats[k] % flex.min(fore_valid), formats[k] % flex.max(fore_valid),
            formats[k] % flex.mean(fore_valid)
        ])

    text.append(table_utils.format(rows, has_header=True, prefix="| ", postfix=" |"))

    if show_flags:
      text.append(_create_flag_count_table(rlist))

  intensity_keys = ('miller_index', 'd', 'intensity.prf.value', 'intensity.prf.variance', 'intensity.sum.value',
                    'intensity.sum.variance', 'background.mean', 'profile.correlation', 'profile.rmsd')
  ...
  centroid_keys = ('miller_index', 'd', 'xyzcal.mm', 'xyzcal.px', 'xyzobs.mm.value', 'xyzobs.mm.variance',
  ...
  def format_column(key, data, format_strings=None):
    ...
    for i in range(len(c_strings[0])):
      ...
      column.append(('%%%is' % len(key)) % ', '.join(
          ('%%%is' % max_element_lengths[j]) % c_strings[j][i] for j in range(len(c_strings))))
    return column
