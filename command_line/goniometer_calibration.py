...
def run(args):
  ...
  from scitbx import matrix
  R_to_mosflm = align_reference_frame(experiments[0].beam.get_s0(), (1.0, 0.0, 0.0),
  ...
  for i in range(len(experiments) - 1):
    ...
    rows.insert(0, (axis_names[i], 'rotation', 'goniometer', depends_on, '%.4f' % axis[0], '%.4f' % axis[1],
                    '%.4f' % axis[2], '.', '.', '.'))
  ...
  loop = cif.model.loop(header=[
      '_axis.id', '_axis.type', '_axis.equipment', '_axis.depends_on', '_axis.vector[1]', '_axis.vector[2]',
...
def write_xoalign_config(file_name, axes, names):
  with open(file_name, 'wb') as f:
    ...
    print >> f, 'GONIOMETER_AXES = ' + '[' + ', '.join('(' + ', '.join('%.5f' % x for x in axis) + ')'
                                                       for axis in axes) + ']'
