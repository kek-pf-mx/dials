...
def export_xds_ascii(integrated_data,
  ...
  distance = matrix.col(Rd * panel.get_origin()).dot(matrix.col(Rd * panel.get_normal()))
  org = Rd * (matrix.col(panel.get_origin()) - distance * matrix.col(panel.get_normal()))
  ...
  fout.write('\n'.join([
      '!FORMAT=XDS_ASCII    MERGE=FALSE    FRIEDEL\'S_LAW=TRUE', '!Generated by dials.export',
      ...
      '!VARIANCE_MODEL= %7.3e %7.3e' % var_model, '!NUMBER_OF_ITEMS_IN_EACH_DATA_RECORD=12', '!ITEM_H=1', '!ITEM_K=2',
      '!ITEM_L=3', '!ITEM_IOBS=4', '!ITEM_SIGMA(IOBS)=5', '!ITEM_XD=6', '!ITEM_YD=7', '!ITEM_ZD=8', '!ITEM_RLP=9',
      '!ITEM_PEAK=10', '!ITEM_CORR=11', '!ITEM_PSI=12', '!END_OF_HEADER', ''
  ]))
  ...
  for j in range(nref):
    ...
    if h == k and k == l:
      ...
      u = (k - l, l - h, h - k)
    q = (matrix.col(u).transpose() * UB.inverse()).normalize().transpose().rotate(axis, phi, deg=True)
