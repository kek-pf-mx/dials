...
class neighbor_analysis(object):
  def __init__(self,
    ...
    assert len(direct) > NEAR, ("Too few spots (%d) for nearest neighbour analysis." % len(direct))
    ...
    if self.histogram_binning == 'log':
      hst = flex.histogram(flex.log10(direct), n_slots=int(len(direct) / self.NNBIN))
    else:
      hst = flex.histogram(direct, n_slots=int(len(direct) / self.NNBIN))
    centers = hst.slot_centers()
    if self.histogram_binning == 'log':
      self.slot_start = flex.double([10**(s - 0.5 * hst.slot_width()) for s in hst.slot_centers()])
    ...
    if False: # to print out the histogramming analysis
      ...
      print >> out, "     mean:      %6.2f +/- %6.2f on N = %d" % (stats.mean(),
    ...
    if percentile is not None:
      ...
      # given multiple
      isel = (self.relative_frequency.as_double() > (max_height_fraction * highest_bin_height)).iselection()
      self.max_cell = (self.tolerance * self.slot_end[int(flex.max(isel.as_double()))])
  ...
  def plot_histogram(self, filename='nn_hist.png', figsize=(12, 8)):
    ...
    plt.bar(
        self.slot_start, self.relative_frequency, align="center", width=self.slot_width, color='black', edgecolor=None)
    ymin, ymax = plt.ylim()
    if self.histogram_binning == 'log':
      ax = plt.gca()
      ax.set_xscale('log')
