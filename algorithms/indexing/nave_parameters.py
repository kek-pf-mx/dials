...
class nave_parameters(object):
  ...
  def __call__(self):
    ...
    print "The mean excursion is %7.3f degrees, r.m.s.d %7.3f" % (mean_excursion,
    ...
    M = minimizer(d_i=dspacings, psi_i=excursion_rad, eta_rad=abs(2. * solution[1]), Deff=d_estimate)
    print "ML: mosaicity FW=%4.2f deg, Dsize=%5.0fA on %d spots" % (M.x[1] * 180. / math.pi, 2. / M.x[0],
                                                                    len(two_thetas))
    ...
    if self.graph_verbose: #params.refinement.mosaic.enable_AD14F7B: # Excursion vs resolution fit
      ...
      plt.title("ML: mosaicity FW=%4.2f deg, Dsize=%5.0fA on %d spots" % (M.x[1] * 180. / math.pi, 2. / M.x[0],
                                                                          len(two_thetas)))
