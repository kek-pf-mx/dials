  ...
  def create(Class, experiments):
    ...
    for e in experiments:
      alg = GaussianRSMaskCalculator(
          e.beam, e.detector, e.goniometer, e.scan, e.profile.delta_b(deg=False), e.profile.delta_m(deg=False))
      result.append(alg)
    return result
...
class GaussianRSIntensityCalculatorFactory(object):
  ...
  @classmethod
  def create(Class, data, detector_space=False, deconvolution=False):
    '''
...
class GaussianRSReferenceCalculatorFactory(object):
  ...
  @classmethod
  def create(Class, experiments, grid_size=5, scan_step=5, grid_method='circular_grid'):
    '''
    Create the intensity calculator
    ...
    if grid_method == 'single':
      ...
      sampler = GridSampler(detector[0].get_image_size(), scan.get_array_range(), (3, 3, num_scan_points))
    elif grid_method == 'circular_grid':
      sampler = CircleSampler(detector[0].get_image_size(), scan.get_array_range(), num_scan_points)
    elif grid_method == 'spherical_grid':
      sampler = EwaldSphereGridSampler(experiments[0].beam, experiments[0].detector, experiments[0].goniometer,
                                       experiments[0].scan, num_scan_points)
    else:
