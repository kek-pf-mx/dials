...
class KaptonAbsorption(object):
  def __init__(self,
    ...
    self.surface2_point_mm = -self.surface_normal * (self.height_mm + self.thickness_mm)
    self.surface3_point_mm = self.surface1_point_mm + self.edge_of_tape_normal * self.half_width_mm
  ...
  def abs_bounding_lines(self, as_xy_ints=False):
    if not hasattr(self, 'segments'): # avoid recomputing the bounding lines on subsequent calls
      ...
      self.surface4_point_mm = self.surface3_point_mm - self.surface_normal * self.thickness_mm
      # get the unit vectors normal to the planes of the absorption minimum and maximum
      self.min_normal = self.tape_dir.cross(self.surface3_point_mm) # i.e. normal to plane through near edge of tape
      self.max_normal = self.tape_dir.cross(self.surface4_point_mm) # i.e. normal to plane through far edge of tape
      self.center_normal_1 = self.tape_dir.cross(self.edge_of_tape_normal) # crosshairs part 1
      self.center_normal_2 = self.surface_normal.cross(self.edge_of_tape_normal) # crosshairs part 2
      ...
      for edge in [min_det_dot, max_det_dot]:
        ...
        d_right_slow = get_intersection(self.size_fast * self.fast, self.slow, edge, self.size_slow)
        # collect the ones that aren't None
        points = set()
        for p in [d_top_fast, d_bottom_fast, d_left_slow, d_right_slow]:
          if p is not None:
            points.add(p)
        assert len(points) <= 2, "There seems to be a plane intersection at a corner of the detector " + \
...
class image_kapton_correction(object):
  ...
  def __call__(self, plot=False):
    def correction_and_within_spot_sigma(params_version, variance_within_spot=True):
      ...
      absorption_sigmas = flex.double() # std dev of corrections for pixels within a spot, default sigma
    ...
    if self.logger is not None:
      self.logger.info("Returning absorption corrections and sigmas for %d spots" % len(corrections))
...
class multi_kapton_correction(object):
  ...
  def __call__(self):
    ...
    for expt, refl in zip(self.experiments, self.reflections.split_by_experiment_id()):
      ...
      refl_valid = refl.select(refl['num_pixels.valid'] > 0 and refl['num_pixels.foreground'] > 0)
      ...
      if len(refl_zero) > 0 and self.params.smart_sigmas:
        ...
        self.corrected_reflections.extend(correct(refl_nonzero, smart_sigmas=True))
        self.corrected_reflections.extend(correct(refl_zero, smart_sigmas=False))
      else:
        self.corrected_reflections.extend(correct(refl_valid, smart_sigmas=self.params.smart_sigmas))
