...
class binner_d_star_cubed(object):
  def __init__(self, d_spacings, target_n_per_bin=25, max_slots=40, min_slots=20):
    ...
    low_res_count = int(math.ceil(min(max(target_n_per_bin, 0.05 * len(d_spacings)), 0.25 * len(d_spacings))))
    bin_step = d_star_cubed_sorted[low_res_count] - d_star_cubed_sorted[0]
    n_slots = int(math.ceil((d_star_cubed_sorted[-1] - d_star_cubed_sorted[0]) / bin_step))
...
def map_to_reciprocal_space(reflections, imageset):
  ...
  reflections = indexer.indexer_base.map_spots_pixel_to_mm_rad(reflections, detector, scan)
  indexer.indexer_base.map_centroids_to_reciprocal_space(reflections, detector, beam, goniometer)
...
def outlier_rejection(reflections):
  ...
  p_prior = 1 / math.sqrt(2 * math.pi * var_prior) * math.exp(-(i_test - flex.mean(intensities_subset))**2 /
                                                              (2 * var_prior))
  #print p_prior
...
def wilson_outliers(reflections, ice_sel=None, p_cutoff=1e-2):
  ...
  if outliers.count(True):
    ...
    inliers = ~outliers
    outliers.set_selected(inliers, wilson_outliers(reflections.select(inliers), ice_sel.select(inliers)))
...
def estimate_resolution_limit(reflections, imageset, ice_sel=None, plot_filename=None):
  ...
  fit = flex.linear_regression(d_star_sq.select(~ice_sel), log_i_over_sigi.select(~ice_sel))
  m = fit.slope()
  c = fit.y_intercept()
  ...
  binner = binner_equal_population(d_star_sq, target_n_per_bin=20, max_slots=20, min_slots=5)
  ...
  for i_slot, slot in enumerate(binner.bins):
    ...
    outliers = wilson_outliers(reflections.select(sel_all), ice_sel=ice_sel.select(sel_all))
  ...
  if plot_filename is not None:
    ...
    ax.scatter(d_star_sq.select(inside), log_i_over_sigi.select(inside), marker='+', color='green')
    ax.scatter(d_star_sq.select(ice_sel), log_i_over_sigi.select(ice_sel), marker='+', color='black')
    ax.scatter(d_star_sq.select(outliers_all), log_i_over_sigi.select(outliers_all), marker='+', color='grey')
    ...
    for i_slot, slot in enumerate(binner.bins):
      if i_slot == 0:
        ax.vlines(uctbx.d_as_d_star_sq(slot.d_max), 0, ax.get_ylim()[1], linestyle='dotted', color='grey')
      ax.vlines(uctbx.d_as_d_star_sq(slot.d_min), 0, ax.get_ylim()[1], linestyle='dotted', color='grey')
...
def estimate_resolution_limit_distl_method1(reflections, imageset, ice_sel=None, plot_filename=None):
...
def estimate_resolution_limit_distl_method2(reflections, imageset, ice_sel=None, plot_filename=None):
...
def points_inside_envelope(d_star_sq, log_i_over_sigi, m_upper, c_upper, m_lower, c_lower):
...
def ice_rings_selection(reflections, width=0.004):
  ...
  if d_spacings:
    ice_filter = filtering.PowderRingFilter(unit_cell, space_group, flex.min(d_spacings), width)
...
def resolution_histogram(reflections, imageset, plot_filename=None):
  ...
  if plot_filename is not None:
    ...
    fig = pyplot.figure()
    ax = fig.add_subplot(1, 1, 1)
    ax.bar(hist.slot_centers() - 0.5 * hist.slot_width(), hist.slots(), width=hist.slot_width())
    ax.set_xlabel("d_star_sq")
...
def stats_single_image(imageset, reflections, i=None, resolution_analysis=True, plot=False, filter_ice=True):
  ...
  if extra_filename is not None:
    log_sum_i_sigi_vs_resolution(reflections, imageset, plot_filename=extra_filename)
  if resolution_analysis and n_spots_no_ice > 10:
    estimated_d_min = estimate_resolution_limit(reflections_all, imageset, ice_sel=ice_sel, plot_filename=filename)
...
def print_table(stats, perm=None, n_rows=None, out=None):
  ...
  print >> out, table_utils.format(rows, has_header=True, prefix="| ", postfix=" |")
...
def plot_stats(stats, filename='per_image_analysis.png'):
  ...
  ax1.scatter(list(i_image), list(n_spots_total), s=5, color='orange', marker='o', alpha=0.4, label='#spots (total)')
  if n_spots_4A is not None:
    ax1.scatter(list(i_image), n_spots_4A, s=5, color='green', marker='o', alpha=0.4, label=u'#spots (to 4\u00c5)')
  ax1.scatter(list(i_image), n_spots_no_ice, s=5, color='blue', marker='o', alpha=0.4, label='#spots (no ice)')
  ...
  ax3.scatter(i_image, total_intensity, s=5, color='grey', marker='o', alpha=0.4, label='Total intensity')
