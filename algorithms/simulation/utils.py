...
def build_prediction_matrix(hkl, mhkl, phkl, hmkl, hpkl, hkml, hkpl, tst=False):
  ...
  d = 0.5 * matrix.sqr((x[2] - x[1], x[4] - x[3], x[6] - x[5], y[2] - y[1], y[4] - y[3], y[6] - y[5], z[2] - z[1],
                        z[4] - z[3], z[6] - z[5]))
  ...
  for j, r in enumerate(refl[1:]):
    dh = matrix.col((r.miller_index[0] - hkl.miller_index[0], r.miller_index[1] - hkl.miller_index[1],
